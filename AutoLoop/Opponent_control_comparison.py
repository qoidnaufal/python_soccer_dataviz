# -*- coding: utf-8 -*-
"""
Created on Tue Apr 18 16:40:59 2023
...
df.rename(columns = {'Shots / on target':'Shots',
                     ...
                     'Unnamed: 88':'Long passes completed',
...
y1_value = f'Opponent {y1} per opponent {y1_control}'
y1_color = 'red'

y2_value = f'Opponent average {y1} per {y1_control}'
y2_color = 'blue'

rolling_average = 4

title_string = f'{team_name} Performance To Deny Opponent Progression'
subtitle_string = f'rolling average every {rolling_average} games | {competition} {season} | data from wyscout'

# adding some variables
df[f'{y1_control}'] = df[f'{y1_control}, %']
...
def cross_df():
    ...
    df_odd[f'Opponent {y1}'] = df_even[y1]
    df_even[f'Opponent {y1}'] =df_odd[y1]
...
df_2[y1_value] = df_2[f'Opponent {y1}'] / df_2[f'Opponent {y1_control}']

# assigning the y2_value
df_2 = df_2.sort_values(by=(['Date', 'Match']), ascending=True).reset_index()
df_2 = df_2.drop(['index'], axis=1)

grouped_opponent = df_2.groupby('Opponent').mean()
grouped_opponent = grouped_opponent.loc[:, f'{y1_value}']
...
grouped_team[f'{y2_value}'] = [grouped_opponent.loc[i] for i in zip(grouped_team['Opponent'])]
...
plt.suptitle(title_string, y=0.951, color='white', fontsize=25)
plt.title(subtitle_string, x=0.484, color='white', fontsize=14)
