...
df = pd.read_csv("~/Documents/LearnPython/tsg2425/data/hgp.csv")
df = df[['Klub', 'HGP minutes', '%']]
...
minutes = df['HGP minutes']
max_minutes = df['HGP minutes'].max()
percentage = df['%']
...
title = ax.set_title("Home Grown Players in BRI Liga 1 2024/25")
